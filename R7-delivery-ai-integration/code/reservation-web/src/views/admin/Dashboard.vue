    ...
    <el-row :gutter="16">
      <el-col :span="14">
        <el-card shadow="never" class="chart-card">
          <div class="chart-title">教室使用率排行（已通过预约占用小时 / 区间可预约小时）</div>
          <div ref="usageRateEl" class="chart-box" />
        </el-card>
      </el-col>
      <el-col :span="10">
        <el-card shadow="never" class="chart-card">
          <div class="chart-title">热门时段分布（已通过预约按开始时段）</div>
          <div ref="timeDistEl" class="chart-box" />
        </el-card>
      </el-col>
    </el-row>
    <el-card shadow="never" class="chart-card">
      <div class="chart-title">月度预约趋势（全部状态预约条数）</div>
...
// 注册看板用到的图表类型与组件（柱状图/折线图/饼图 + 提示框/网格/图例 + Canvas 渲染）
  ...
  trendChart.setOption(
    {
      ...
      xAxis: { type: 'category', data: list.map((r) => r.month) },
      yAxis: { type: 'value', name: '预约条数', minInterval: 1 },
      series: [
        {
          type: 'line',
