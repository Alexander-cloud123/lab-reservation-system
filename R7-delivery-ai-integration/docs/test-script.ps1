# R7 接口测试脚本（R1-R6 共 210 条基线回归 + R7 AI 模块新增 28 条：权限/开关/双开关/边界/只读/开启路径功能与降级）
# 覆盖基线：R1 21 + R2 30 + R3 33 + R4 39 + R5 41 + R6 46 = 210 条；R7 新增 T213-T240 = 28 条
# 运行模式：交付配置（yml ai.enable=false + db ai_enable=false）下自动执行关闭路径用例；
#          联调模式（yml ai.enable=true）下自动执行开启路径功能/降级用例（无 AGNES_API_KEY 时走规则降级，同样可用）
# 依赖：后端已启动于 http://localhost:8080；reservation-mysql 容器运行中
# 可复跑：注册/测试用户带时间戳后缀；测试新增教室结束时删除；临时用户与测试预约由 SQL 清理恢复初始基线
# 依赖：后端已启动于 http://localhost:8080；reservation-mysql 容器运行中
...
# R7 修复：晚间 20 点后运行脚本时“2 小时后”会越过营业时间边界（22:00）导致创建失败——此时改用明日 8:00（仍在 24h 即将开始窗口内），消除时间依赖保证可复跑
$twoH = (Get-Date).AddHours(2)
if ($twoH.Hour -ge 22 -or $twoH.Hour -lt 8) {
  $twoH = (Get-Date).Date.AddDays(1).AddHours(8)
}
...
Write-Host ''
Write-Host '========== 七-补、R7 AI 模块（正常/边界/异常/权限/降级/开关）=========='

# ===== 1. 权限校验 =====
Write-Host '== 1. 权限校验 =='
# 未登录 401（4 接口）
$r = Invoke-Api 'POST' '/api/ai/recommend' @{ userId = 2 }
Add-Case 'T213' 'AI推荐-未登录访问' '权限校验' "HTTP=$($r.Status)" 'HTTP=401'
$r = Invoke-Api 'POST' '/api/ai/parse-reservation' @{ text = '测试' }
Add-Case 'T214' 'AI解析-未登录访问' '权限校验' "HTTP=$($r.Status)" 'HTTP=401'
$r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '测试' }
Add-Case 'T215' 'AI问答-未登录访问' '权限校验' "HTTP=$($r.Status)" 'HTTP=401'
$r = Invoke-Api 'POST' '/api/ai/compliance-check' @{ purpose = '测试' }
Add-Case 'T216' 'AI合规-未登录访问' '权限校验' "HTTP=$($r.Status)" 'HTTP=401'
# 登录即可：学生/管理员均可访问（上报确认口径）
$r = Invoke-Api 'POST' '/api/ai/recommend' @{ userId = 2 } $stuToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
Add-Case 'T217' 'AI推荐-学生Token可访问' '权限校验' "HTTP=$($r.Status) code=$($b.code)" 'HTTP=200'
$r = Invoke-Api 'POST' '/api/ai/compliance-check' @{ purpose = '课程设计' } $adminToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
Add-Case 'T218' 'AI合规-管理员Token可访问' '权限校验' "HTTP=$($r.Status) code=$($b.code)" 'HTTP=200'
# 伪造 Token 401
$r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '怎么预约' } 'eyJhbGciOiJIUzI1NiJ9.tampered'
Add-Case 'T219' 'AI问答-伪造Token访问' '权限校验' "HTTP=$($r.Status)" 'HTTP=401'

# ===== 3. 边界与异常（空参友好处理，不 500）=====
Write-Host '== 3. 边界与异常 =='
$r = Invoke-Api 'POST' '/api/ai/recommend' @{} $stuToken
Add-Case 'T225' 'AI推荐-缺userId' '边界场景' "HTTP=$($r.Status) body=$($r.Body)" '非500友好处理'
$r = Invoke-Api 'POST' '/api/ai/parse-reservation' @{} $stuToken
Add-Case 'T226' 'AI解析-缺text' '边界场景' "HTTP=$($r.Status) body=$($r.Body)" '非500友好处理'
$r = Invoke-Api 'POST' '/api/ai/chat' @{} $stuToken
Add-Case 'T227' 'AI问答-缺question' '边界场景' "HTTP=$($r.Status) body=$($r.Body)" '非500友好处理'
$r = Invoke-Api 'POST' '/api/ai/compliance-check' @{} $stuToken
Add-Case 'T228' 'AI合规-缺purpose' '边界场景' "HTTP=$($r.Status) body=$($r.Body)" '非500友好处理'
$r = Invoke-Api 'POST' '/api/ai/chat' 'not-json' $stuToken
Add-Case 'T229' 'AI问答-非法JSON' '异常操作' "HTTP=$($r.Status) body=$($r.Body)" 'HTTP=400'

# ===== 4. 只读校验（AI 不写业务数据）=====
Write-Host '== 4. 只读校验（调用前后预约数不变）=='
$r = Invoke-Api 'GET' '/api/reservation/mine?page=1&size=50' $null $stuToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
$totalBefore = if ($b.data) { $b.data.total } else { -1 }
$null = Invoke-Api 'POST' '/api/ai/recommend' @{ userId = 2 } $stuToken
$null = Invoke-Api 'POST' '/api/ai/parse-reservation' @{ text = '明天下午2点 40人 机房' } $stuToken
$null = Invoke-Api 'POST' '/api/ai/chat' @{ question = '怎么预约' } $stuToken
$null = Invoke-Api 'POST' '/api/ai/compliance-check' @{ purpose = '商业推销' } $stuToken
$r = Invoke-Api 'GET' '/api/reservation/mine?page=1&size=50' $null $stuToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
$totalAfter = if ($b.data) { $b.data.total } else { -1 }
$readonlyOk = ($totalBefore -eq $totalAfter)
Add-Case3 'T230' 'AI只读-调用前后预约数不变' '数据安全' "before=$totalBefore after=$totalAfter ok=$readonlyOk" 'ok=True'

# ===== 5. AI 开关路径用例（按环境自动分支：开启路径 / 关闭路径）=====
Write-Host '== 5. AI 开关路径用例 =='
$aiOn = $false
$r = Invoke-Api 'POST' '/api/ai/recommend' @{ userId = 2 } $stuToken
if ($r.Status -eq 200 -and $r.Body) { $bb = $r.Body | ConvertFrom-Json; if ($bb.data -and $bb.data.enabled -eq $true) { $aiOn = $true } }
if ($aiOn) {
  Write-Host '-- 开启路径（yml ai.enable=true + db ai_enable=true）--'
  # ---- 正常流程（无 AGNES_API_KEY 时自动降级规则模拟，仍返回可用结果）----
  Write-Host '-- 正常流程 --'
  $r = Invoke-Api 'POST' '/api/ai/recommend' @{ userId = 2 } $stuToken
  $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
  $items = @(); if ($b.data -and $b.data.recommendations) { $items = $b.data.recommendations }
  $recOk = ($items.Count -ge 1 -and $null -ne $items[0].name -and $null -ne $items[0].reason)
  Add-Case3 'T231' 'AI推荐-返回Top3含教室与理由' '正常流程' "HTTP=$($r.Status) count=$($items.Count) ok=$recOk" 'ok=True'
  $r = Invoke-Api 'POST' '/api/ai/parse-reservation' @{ text = '明天下午2点 40人 机房 做课程设计' } $stuToken
  $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
  $pOk = $false
  if ($b.data) { $pOk = ($b.data.date -match '^\d{4}-\d{2}-\d{2}$' -and $b.data.startTime -eq '14:00' -and $b.data.endTime -eq '16:00' -and $b.data.capacity -eq 40 -and $b.data.roomType -eq '机房') }
  Add-Case3 'T232' 'AI解析-结构化字段正确' '正常流程' "HTTP=$($r.Status) date=$($b.data.date) start=$($b.data.startTime) end=$($b.data.endTime) cap=$($b.data.capacity) type=$($b.data.roomType) ok=$pOk" 'ok=True'
  $r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '怎么取消预约？' } $stuToken
  $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
  $ansLen = if ($b.data -and $b.data.answer) { $b.data.answer.Length } else { 0 }
  $ansOk = ($ansLen -gt 0)
  Add-Case3 'T233' 'AI问答-相关问题返回FAQ' '正常流程' "HTTP=$($r.Status) len=$ansLen ok=$ansOk" 'ok=True'
  $r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '推荐一部电影' } $stuToken
  $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
  $scoped = ($b.data -and $b.data.answer -like '*只能解答*')
  Add-Case3 'T234' 'AI问答-场景外预设话术' '正常流程' "HTTP=$($r.Status) scoped=$scoped" 'scoped=True'
  $r = Invoke-Api 'POST' '/api/ai/compliance-check' @{ purpose = '商业推销活动' } $stuToken
  $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
  $cOk = ($b.data -and $b.data.compliant -eq $false -and $b.data.reason -like '*违规关键词*')
  Add-Case3 'T235' 'AI合规-违规词判定' '正常流程' "HTTP=$($r.Status) compliant=$($b.data.compliant) ok=$cOk" 'ok=True'
  $r = Invoke-Api 'POST' '/api/ai/compliance-check' @{ purpose = '课程设计小组讨论' } $stuToken
  $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
  $cOk2 = ($b.data -and $b.data.compliant -eq $true)
  Add-Case3 'T236' 'AI合规-正常用途通过' '正常流程' "HTTP=$($r.Status) compliant=$($b.data.compliant) ok=$cOk2" 'ok=True'

  Write-Host '-- 降级 --'
  $r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '怎么预约？' } $stuToken
  $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
  $dgMsg = if ($b.data) { $b.data.message } else { '' }
  Add-Case3 'T237' 'AI问答-无密钥自动降级本地规则' '降级' "HTTP=$($r.Status) msg=$dgMsg" '本地规则模式'
  $hitRate = $false
  for ($i = 1; $i -le 25; $i++) {
    $r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '怎么预约？' } $stuToken
    $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
    if ($b.data -and $b.data.message -like '*频繁*') { $hitRate = $true; break }
    Start-Sleep -Milliseconds 80
  }
  Add-Case3 'T238' 'AI问答-限流自动降级（RPM=20）' '降级' "HTTP=$($r.Status) hit=$hitRate msg=$($b.data.message)" 'hit=True'
  $sw = [System.Diagnostics.Stopwatch]::StartNew()
  $r = Invoke-Api 'POST' '/api/ai/recommend' @{ userId = 2 } $stuToken
  $sw.Stop()
  $ms = $sw.ElapsedMilliseconds
  $msOk = ($ms -le 3000)
  Add-Case3 'T239' 'AI推荐-响应时间<=3s' '正常流程' "HTTP=$($r.Status) ms=$ms ok=$msOk" 'ok=True'
  $noPwd = (-not $r.Body.Contains('password'))
  Add-Case3 'T240' 'AI推荐-响应不含password' '数据安全' "noPwd=$noPwd" 'noPwd=True'
  # T241 双开关反向：开启模式下 SQL 关 db（yml=true + db=false → 整体关闭），验证后恢复 db=true
  docker exec reservation-mysql mysql -uroot -proot reservation -e "UPDATE ai_config SET config_value='false' WHERE config_key='ai_enable';" 2>$null | Out-Null
  $r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '怎么预约' } $stuToken
  $b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
  $en = 'null'; if ($b.data) { $en = $b.data.enabled }
  Add-Case3 'T241' 'AI问答-双开关AND（yml开+db关=关）' '开关' "HTTP=$($r.Status) code=$($b.code) enabled=$en" 'enabled=False'
  docker exec reservation-mysql mysql -uroot -proot reservation -e "UPDATE ai_config SET config_value='true' WHERE config_key='ai_enable';" 2>$null | Out-Null
} else {
  Write-Host '-- 关闭路径（ai.enable=false 交付配置）--'
# ===== 2. 开关与双开关口径（交付配置 ai.enable=false）=====
Write-Host '== 2. 开关与双开关口径 =='
$r = Invoke-Api 'POST' '/api/ai/recommend' @{ userId = 2 } $stuToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
$en = 'null'; $msg = ''
if ($b.data) { $en = $b.data.enabled; $msg = $b.data.message }
Add-Case3 'T220' 'AI推荐-关闭路径enabled=false友好提示' '开关' "HTTP=$($r.Status) code=$($b.code) enabled=$en msg=$msg" 'enabled=False'
$r = Invoke-Api 'POST' '/api/ai/parse-reservation' @{ text = '明天下午2点机房' } $stuToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
$en = 'null'; if ($b.data) { $en = $b.data.enabled }
Add-Case3 'T221' 'AI解析-关闭路径enabled=false' '开关' "HTTP=$($r.Status) code=$($b.code) enabled=$en" 'enabled=False'
$r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '怎么预约' } $stuToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
$en = 'null'; if ($b.data) { $en = $b.data.enabled }
Add-Case3 'T222' 'AI问答-关闭路径enabled=false' '开关' "HTTP=$($r.Status) code=$($b.code) enabled=$en" 'enabled=False'
$r = Invoke-Api 'POST' '/api/ai/compliance-check' @{ purpose = '课程设计' } $stuToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
$en = 'null'; if ($b.data) { $en = $b.data.enabled }
Add-Case3 'T223' 'AI合规-关闭路径enabled=false' '开关' "HTTP=$($r.Status) code=$($b.code) enabled=$en" 'enabled=False'
# 双开关 AND 语义：yml=false + db=true 仍整体关闭（验证后恢复 db=false）
docker exec reservation-mysql mysql -uroot -proot reservation -e "UPDATE ai_config SET config_value='true' WHERE config_key='ai_enable';" 2>$null | Out-Null
$r = Invoke-Api 'POST' '/api/ai/chat' @{ question = '怎么预约' } $stuToken
$b = $null; if ($r.Body) { try { $b = $r.Body | ConvertFrom-Json } catch {} }
$en = 'null'; if ($b.data) { $en = $b.data.enabled }
Add-Case3 'T224' 'AI问答-双开关AND（yml关+db开=关）' '开关' "HTTP=$($r.Status) code=$($b.code) enabled=$en" 'enabled=False'
docker exec reservation-mysql mysql -uroot -proot reservation -e "UPDATE ai_config SET config_value='false' WHERE config_key='ai_enable';" 2>$null | Out-Null

  Write-Host '开启路径功能/降级用例（T231-T241）在联调模式实测通过，详见 docs/test-report.md；交付配置下自动跳过。'
}
