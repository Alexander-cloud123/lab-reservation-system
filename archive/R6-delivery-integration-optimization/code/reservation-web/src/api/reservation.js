...
/** 管理端：全量预约查询（status/startDate/endDate/keyword/classroomId） */
export function pageManageReservations(params) {
  return request.get('/reservation/manage', { params })
}

/** 管理端：预约记录导出（R6 预约记录页，xlsx 文件流，需手动触发浏览器下载） */
export function exportReservations(params) {
  return request.get('/reservation/export', { params, responseType: 'blob' })
}
